...
| **Opt-in usage stats** | *(no tables — reads existing ones)* | active; off unless the operator opts in during `rumi setup`. `bot/shared/utils/telemetry.js` posts three counts a day to the project; see [SETUP.md](../SETUP.md#sharing-usage-stats-optional-off-by-default). |
